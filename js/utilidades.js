...
function Fecha_aNum(fecha) {
     var fecha_gsbase = 0;
     if (fecha != ""){
          lst_fecha = fecha.split("/", -1);
          sdia = lst_fecha[0];
          smes = lst_fecha[1];
          sano = lst_fecha[2];
...
function Num_aFecha(fecha_gsbase) {
     var fecha = "";
     if (fecha_gsbase > 0){
          ano = parseInt(fecha_gsbase / 372);
          dias = parseInt(fecha_gsbase - (ano * 372));
          mes = parseInt(dias / 31);

          //alert(mes);
          var diasMes = new Date(ano, mes + 1, 0).getDate();
          //alert(diasMes);

          dia = parseInt(dias - (mes * 31));
          ano = 2000 + ano;
          dia += 1;
          mes += 1;
...
function format_fecha_local(fecha){
     var fecha_devuelta = "";
...
function format_fecha_global(fecha){
     var fecha_devuelta = "";
     fecha_devuelta = fecha.split('/').reverse().join('-');
     return fecha_devuelta;
}
